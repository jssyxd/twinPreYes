...
def poll_fill(client, order_id: str, *, attempts: int = 6, sleep=None, sleep_seconds: float = 1.0,
    ...
    for attempt in range(1, max(1, attempts) + 1):
        ...
        if status in TERMINAL_STATUSES or (status in RESTING_STATUSES and matched > 0 and matched == original):
            ...
            avg = None
            notional = None
            avg_source = "unavailable"
            if trades:
                report = fill_trades(client, str(order_id), token_id=_field(order, "asset_id"))
                if report is not None:
                    avg = report["avg"]
                    notional = report["notional"]
                    avg_source = "trades"
            if avg is None and matched == ZERO and price is not None:
                # 一股未成 ⇒ 没有"实际成交价"可言；保留既有回退（pre-existing behaviour，仅供人读，
                # 成本必为 0）。③（2026-09-13）：**有成交却读不到台账**时**绝不**再用限价冒充成交价
                # —— 那正是"账本按股数 × 限价高估成本"的源头；此时 avg/notional 都留 None，
                # 由调用方按"实际未知"处理并落审计（cost_source=unavailable）。
                avg = Decimal(str(price))
                avg_source = "no_fill_legacy"
            filled = matched
            return {"ok": True, "status": status, "attempts": attempt, "terminal": True,
                    "filled_shares": filled, "avg_price": avg, "notional_usdc": notional,
...
#: 成交量台账里"这一行属于哪张单"的字段名（2026-09-13，③）：真实链上台账用的是
#: ``taker_order_id`` / ``maker_order_id``，旧代码只认 ``orderID`` / ``order_id`` / ``id`` ⇒
#: 一行都匹配不上 ⇒ ``average_fill_price`` 返回 None ⇒ 成本回退成**限价**（账本高估）。
ORDER_ID_FIELDS = ("orderID", "order_id", "taker_order_id", "maker_order_id",
                   "takerOrderId", "makerOrderId")


def fill_trades(client, order_id: str, *, token_id: str | None = None) -> dict | None:
    """该订单在成交量台账里的**实际成交**汇总：``{shares, notional, avg}``；读不到 ⇒ ``None``。

    ③（2026-09-13）根因修复：join 字段名放宽到真实台账用名（``taker_order_id`` 等）。旧实现只认
    ``orderID``/``order_id``/``id``，且第二道 ``order_id is None ⇒ continue`` 会把**真实行**整片
    跳过 ⇒ ``avg`` 为 None ⇒ ``poll_fill`` 回退限价 ⇒ 账本按 ``股数 × 限价`` 高估成本
    （实证 seoul：链上实付 5.04582 被记成 32.375 × 0.31 = 10.03625）。

    ``notional``（名义额 USDC = Σ size×price）是**链上真实扣款**，优先于 ``avg``（加权均价）用于记账。
    纯读、best effort：台账不可读 ⇒ ``None``（调用方**绝不**据此回退成限价）。
    """
    try:
        rows = client.get_trades() or []
    except Exception:  # noqa: BLE001 - 台账不可读不是成交，绝不能据此伪造价格
        return None
    matched: list[Any] = []
    for row in rows:
        oid = _field(row, *ORDER_ID_FIELDS)
    ...
    return {"shares": shares,
            "notional": cost.quantize(QTY),
            "avg": (cost / shares).quantize(QTY)}
...
def average_fill_price(client, order_id: str, *, token_id: str | None = None) -> Decimal | None:
    ...
    Thin wrapper over :func:`fill_trades` (kept for existing callers/tests).
    """
    report = fill_trades(client, order_id, token_id=token_id)
    return None if report is None else report["avg"]
...
#: live **退场**通道（2026-09-13）—— 记录在审计行的 ``exit_channel`` 上，标明这次 SELL 是引擎的哪一处
#: 退场触发的。与 ``ENTRY_CHANNELS`` 同样只接受白名单值（未知值不盖章，绝不落调用方的字符串）。
#: 字面量副本（同 ENTRY_CHANNELS 的理由：本模块必须能独立 import）。
EXIT_CHANNEL_EARLY_STOP = "early_stop"       # 抢先止损（evaluate_early_stop_loss）
EXIT_CHANNEL_REFIRE_LIQ = "refire_liq"      # 追火旧桶清算（record_refire）
EXIT_CHANNEL_SLEEVE_TIMEOUT = "sleeve_timeout"   # 未破位 sleeve 超时退场
EXIT_CHANNEL_BREACH_RC = "breach_rc"        # METAR 破位风控清算
EXIT_CHANNEL_TAKE_PROFIT = "take_profit"    # 50% 阶梯止盈 (bid >= 1.5 * entry)
EXIT_CHANNEL_ABORTION_RC = "abortion_rc"    # 气象不可逆夭折证伪清算
EXIT_CHANNELS = (EXIT_CHANNEL_EARLY_STOP, EXIT_CHANNEL_REFIRE_LIQ,
                 EXIT_CHANNEL_SLEEVE_TIMEOUT, EXIT_CHANNEL_BREACH_RC,
                 EXIT_CHANNEL_TAKE_PROFIT, EXIT_CHANNEL_ABORTION_RC)


def _exit_channel_label(value) -> str | None:
    """白名单化的退场通道标签（未知 ⇒ ``None`` ⇒ 不盖章）。"""
    return str(value) if value in EXIT_CHANNELS else None

#: 审计行里**不可伪造**的键（F-B/F-C，2026-09-12；2026-09-13 扩到 SELL 出场）。它们描述"这笔单事实上
#: 是什么"（走的哪条 API/多少钱/哪边/哪条通道/哪个窗口/哪个签名域），因此**只能由本模块的计算值或专用
#: 入参盖章**：调用方通过 ``audit_extra`` 传进来的同名键一律先剔除，绝不覆盖权威值，也绝不出现在
#: ``deny`` 行里（此前 5 条 early-deny 路径会把调用方注入的 ``entry_channel``/``leg_window``
#: 原样带走 ⇒ 审计日志里"哪条通道被拒"可被伪造；生产链路上端口自己算，故无实害）。
#: ``side`` / ``exit_channel`` / ``sell_floor`` 于 2026-09-13 加入：live 真实 SELL 出场要求
#: "side=SELL / exit_channel / sell_floor" 不可被 ``audit_extra`` 伪造（此前 ``side`` 会被
#: ``params.update(extra_params)`` 覆盖 ⇒ 一条卖单能被记成买单）。
SPOOFABLE_AUDIT_KEYS = (
    "order_api", "amount", "amount_unit", "side",
    "entry_channel", "leg_window", "window_source", "next_entry_window",
    "neg_risk", "neg_risk_source",
    "exit_channel", "sell_floor",
)
...
#: SELL（退场）方向的拒单原因（2026-09-13）。BUY 用 ``taker_cap_required`` / ``above_cap``；
#: SELL 用下面三条：地板缺失 / 地板非法 / 限价低于地板。三条都是**拒单**，绝不降级成无保护卖单。
EXIT_FLOOR_REQUIRED = "exit_floor_required"
EXIT_FLOOR_INVALID = "exit_floor_invalid"
EXIT_BELOW_FLOOR = "below_exit_floor"
...
def _taker_floor(value) -> Decimal | None:
    """SELL 侧的**保护地板**：一个显式的 ``0 < floor <= 1``，否则 ``None``。

    退场（SELL）方向的价格护栏与 BUY 的 ``cap`` 互为镜像：``cap`` 是"绝不多付"，
    ``floor`` 是"绝不少卖"。缺失 / 不可解析 / ``<= 0`` / ``> 1``（超出场所价格域）一律
    ``None`` ⇒ 调用方**拒单**（``exit_floor_required`` / ``exit_floor_invalid``），
    **绝不**回退成无保护卖单 —— 与 ``_taker_cap`` 同一条"最后一道防线不依赖调用方自觉"的原则。
    （2026-09-13：live 真实 SELL 出场通道，操作者拍板地板 ``live_sell_floor = 0.05``。）
    """
    try:
        out = _dec(value, "floor", allow_zero=False)
    except ValueError:
        return None
    return out if out <= ONE else None
...
def execute_leg(client, *, token_id: str, side: str, price, size, book=None, tick=None,
                ...
                neg_risk_source: str | None = None, floor=None,
                exit_channel: str | None = None) -> dict:
    ...
    * ``taker=True`` — **taker / aggressive** (the operator's LIVE rule): the incoming limit is
      ...
      limits, **mirrored per side** (2026-09-13):

      - ``BUY``: an explicit ``0 < cap <= 1`` is **required** (missing / unparseable /
        non-positive / ``> 1`` ⇒ refuse ``taker_cap_required``) and the leg's ask ``cap`` is
        *refused* (``above_cap``) rather than chased.
      - ``SELL`` (the live exit direction): an explicit ``0 < floor <= 1`` is **required**
        (missing ⇒ ``exit_floor_required``; unparseable / ``<= 0`` / ``> 1`` ⇒
        ``exit_floor_invalid``) and a limit **below** that floor is refused (``below_exit_floor``).
        ``cap`` is a BUY-side concept and is **not** consulted for a SELL.  A missing/invalid
        floor must never fall back to an unprotected sell — that is the whole point of the
        floor (operator rule: ``live_sell_floor = 0.05``, ``bid < floor`` ⇒ do not sell).

      A limit outside ``(0, 1]`` is refused (``price_out_of_range``) and a limit that is not
      tick-aligned is refused (``price_not_on_tick``) instead of being moved.
    ...
    ``amount_unit`` / ``side`` and the SELL-side ``exit_channel`` / ``sell_floor`` — are
    **unforgeable** (F-B/F-C, 2026-09-12; F-F, 2026-09-13): every same-named key arriving
    ...
    out = {"ok": False, "status": "not_started", "order_id": None, "filled_shares": ZERO,
           ...
           "market_amount": None, "amount_unit": None, "side": side, "sell_floor": None,
           # ③（2026-09-13）：实际成交证据（名义额 USDC / 成本来源）；未成交时为 None
           "notional_usdc": None, "cost_source": None}
    ...
    # 不可伪造字段（F-B/F-C，2026-09-12；F-F 2026-09-13 加入 side/exit_channel/sell_floor）：先于一切
    # 使用**剔除**调用方注入的同名键 —— 包括下面每条 early-deny 行的 ``_deny_audit(..., {**extra_params,
    # ...})``。权威值只在下面按计算值/专用入参重新盖章（缺失 ⇒ 该键不出现，绝不落调用方的值）。
    for _spoofable in SPOOFABLE_AUDIT_KEYS:
        extra_params.pop(_spoofable, None)
    #: SELL 地板（权威值只在 SELL 分支里赋）—— deny 行与审计行都从它盖章，绝不取调用方塞的值
    floor_dec: Decimal | None = None
    ...
    if taker:
        ...
        if side == "SELL":
            # ---- SELL（live 退场）：价格护栏是**地板**，不是 cap（2026-09-13）----------
            # 地板缺失/非法 ⇒ 拒单，绝不回退成无保护卖单（操作者口径：bid < floor 不卖）。
            floor_dec = _taker_floor(floor)
            if floor_dec is None:
                reason = EXIT_FLOOR_REQUIRED if floor is None else EXIT_FLOOR_INVALID
                _deny_audit(audit_path, reason,
                            {**extra_params, "token_id": str(token_id), "price": str(want),
                             "size": str(shares), "floor": (str(floor) if floor is not None else None),
                             **({"exit_channel": _exit_channel_label(exit_channel)}
                                if _exit_channel_label(exit_channel) else {})})
                return {**out, "status": reason,
                        "detail": (f"SELL refused: floor={floor!r} is not a usable protection "
                                   f"floor (need 0 < floor <= 1) — nothing sent, no unprotected "
                                   f"sell")}
            if want > ONE or want <= ZERO:
                ...
                _deny_audit(audit_path, "price_out_of_range",
                            {**extra_params, "price": str(want), "sell_floor": str(floor_dec)})
                return {**out, "status": "price_out_of_range",
                        "detail": f"taker limit {want} is outside (0, 1] — refuse"}
            if want < floor_dec:
                _deny_audit(audit_path, EXIT_BELOW_FLOOR,
                            {**extra_params, "price": str(want), "sell_floor": str(floor_dec),
                             "size": str(shares),
                             **({"exit_channel": _exit_channel_label(exit_channel)}
                                if _exit_channel_label(exit_channel) else {})})
                return {**out, "status": EXIT_BELOW_FLOOR,
                        "detail": (f"SELL limit {want} < sell floor {floor_dec} — refuse "
                                   f"(unprotected sell), nothing sent")}
        else:
        ...
        out.update({"limit_price": str(limit), "clamped": False,
                    ...
                    "sell_floor": str(floor_dec) if floor_dec is not None else None})
    else:
    ...
    # live 退场通道（2026-09-13，F-F）：SELL 侧同样只认**专用入参**。一条单只属于一条通道
    # （入场 ``entry_channel`` 与出场 ``exit_channel`` 互斥），因此二者不同时报。
    exit_label = _exit_channel_label(exit_channel)
    if exit_label is not None:
        params["exit_channel"] = exit_label
        if leg_window is not None:
            params["leg_window"] = str(leg_window)
    if floor_dec is not None:
        params["sell_floor"] = str(floor_dec)
    if side is not None:
    ...
    notional = fill.get("notional_usdc")
    # ③（2026-09-13）成本 = **实际成交**：优先名义额 USDC（链上真实扣款），其次 股数 × 实际成交均价。
    # 两者都取不到（有成交但台账读不到）⇒ cost=0 + cost_source=unavailable —— **绝不**回退
    # ``股数 × 限价``（那正是被修掉的缺陷）。纯决策路径（无成交）保持 cost=0。
    if notional is not None:
        cost = _dec(notional, "notional_usdc")
        cost_source = "notional_usdc"
    elif filled > ZERO and avg is not None:
        cost = (filled * avg).quantize(QTY)
        cost_source = "shares_x_avg_price"
    else:
        cost = ZERO
        cost_source = "unavailable" if filled > ZERO else "no_fill"
    result = {**out, "ok": True, "order_id": order_id, "status": fill["status"],
              "filled_shares": filled, "avg_price": avg, "cost": cost,
              "notional_usdc": notional, "cost_source": cost_source,
