...
from live import exit as live_exit
from live.port import PortRefused, get_port
from paper_capital import close_leg_at_best_bid as _paper_close_leg_at_best_bid, reserve
...
# --------------------------------------------------------------------------- #
# live 真实 SELL 出场通道（2026-09-13）—— 消除 live 下的「虚拟平仓」
# --------------------------------------------------------------------------- #
# 本模块有 **4 处** 腿退场调用点（早停 / 追火旧桶清算 / sleeve 超时 / 破位风控清算）。它们原先
# 一律走 ``paper_capital.close_leg_at_best_bid``（按 best_bid 记账 + 标 settled，**不下真实卖单**）；
# 在 live 下这会让真币留在钱包而账本以为已出场（账实背离）。现在每处都写成
#
#     if _cycle_is_live(cfg):
#         <真实 FAK 卖：live/exit.py>
#     else:
#         <原 paper 平仓，逐字保留>
#
# 并由下面的两层不变式保证 live 下**不可能**发生虚拟平仓：
#   ① 静态：4 个 ``close_leg_at_best_bid`` 调用点都只在 ``if ... live ... else:`` 的 **else** 分支里
#      （tests_live_exit.py 用 AST 断言）；
#   ② 运行时：被调用的 ``close_leg_at_best_bid`` 名字本身是**守卫包装**（见下），它在 live 轮次里
#      遇到仍有真实股数的腿 ⇒ 记审计行 + 抛 ``LiveVirtualCloseRefused``，绝不静默按 bid 记账。
#
#: 当前轮次的模式（``run_cycle`` 顶部绑定；缺省 ``paper`` ⇒ 既有行为不变）。放成模块级是因为
#: 守卫包装必须知道"这一轮是不是 live"，而 ``close_leg_at_best_bid`` 的签名里没有 cfg。
_CYCLE_MODE: str = "paper"
#: 当前轮次的事件日志路径（守卫包装记审计行的落点；同样由 ``run_cycle`` 绑定）。
_CYCLE_LOG_PATH: str = "data/yes2re_events.jsonl"


class LiveVirtualCloseRefused(RuntimeError):
    """live 下有人试图对真实持仓做虚拟平仓 —— 红线，拒绝并响亮报错。"""


def set_cycle_mode(mode: Any, log_path: Any = None) -> str:
    """绑定本轮模式（``run_cycle`` 调用；测试/运维也可显式设置）。返回归一化后的模式。"""
    global _CYCLE_MODE, _CYCLE_LOG_PATH
    _CYCLE_MODE = str(mode or "paper").strip().lower() or "paper"
    if log_path:
        _CYCLE_LOG_PATH = str(log_path)
    return _CYCLE_MODE
...
def _cycle_is_live(cfg: dict[str, Any] | None = None) -> bool:
    """本轮是否 live：给了 cfg 就以 cfg['mode'] 为准，否则用 ``run_cycle`` 绑定的轮次模式。"""
    if cfg is not None and ("mode" in cfg):
        return str(cfg.get("mode") or "paper").strip().lower() == "live"
    return _CYCLE_MODE == "live"
...
def close_leg_at_best_bid(
    ...
    *,
    ...
    """**守卫包装**过的 paper 虚拟平仓（不变式②的运行时哨兵）。

    paper 模式：逐字透传给 ``paper_capital.close_leg_at_best_bid``（行为、返回值、账本完全不变）。
    live 模式：只要这腿还持有真实股数（未 settled 且 shares > 0），就说明有人绕过了 mode 分支 ——
    这是"真实持仓被虚拟平仓"的红线。此时记一条 ``virtual_close_refused`` 事件并抛
    :class:`LiveVirtualCloseRefused`，**绝不**按 best_bid 记账（fail-closed 且响亮）。
    """
    if _CYCLE_MODE == "live" and isinstance(leg, dict) and not leg.get("settled"):
        try:
            shares = Decimal(str(leg.get("shares") or 0))
        except Exception:  # noqa: BLE001 — 解析不了也按"有股数"处理（保守）
            shares = ZERO
        if shares > ZERO:
            log_event(
                _CYCLE_LOG_PATH,
                {
                    "type": "virtual_close_refused",
                    "reason": "live_virtual_close_forbidden",
                    "closed_by": closed_by,
                    "token_id": leg.get("token_id"),
                    "leg": leg.get("leg"),
                    "shares": str(shares),
                    "detail": ("live 模式下不得虚拟平仓：真实持仓退场必须走真实 FAK 卖出场通道"
                               "（live/exit.py）"),
                },
            )
            raise LiveVirtualCloseRefused(
                "refusing a virtual (paper) close of a real position while mode=live: "
                f"leg={leg.get('leg')!r} token={leg.get('token_id')!r} shares={shares} "
                f"closed_by={closed_by!r}"
            )
    return _paper_close_leg_at_best_bid(state, leg, books=books, closed_by=closed_by,
...
def _live_exit_leg(
    ...
    *,
    ...
    leg_window: str | None = None,
    attempts: int = 0,
) -> dict[str, Any]:
    """一处 live 腿退场的统一入口（真实 FAK 卖；见 ``live/exit.py``）。

    只做三件事：把引擎的 ``log_event`` 接进通道、把本轮模式显式声明为 live（守卫前提）、
    然后交给 ``live.exit.live_exit_leg`` 完成 决策→撤真实挂单→SELL→按真实成交记账→审计。
    """
    set_cycle_mode("live")
    return live_exit.live_exit_leg(
        cfg=cfg, state=state, pos=pos, leg=leg, books=books, channel=channel,
        shares_override=shares_override,
        now_utc=now_utc, leg_window=leg_window, attempts=attempts,
        log=lambda payload: log_event(cfg.get("log_path"), payload),
    )


def _retry_pending_exits(cfg: dict[str, Any], state: dict[str, Any], now_utc: datetime) -> None:
    """live 续卖扫（**不是**第 5 处虚拟平仓）：把 ``pos["pending_exit"]`` 里的余量再卖一次。

    操作者规则 ④/⑤：部分成交的余量、以及被拒/无买盘而延期的退场，必须**下一轮重试**，绝不丢弃。
    早停 / 破位 / sleeve 三条通道天然会在下一轮重新触发；只有 追火旧桶清算 是"一次性"事件，
    余量只能靠这里兜住 —— 因此四条通道的延期都登记 ``pending_exit``，由本扫描统一续卖。

    paper 模式：整段直接返回（行为逐字不变）。live 模式：逐条真实卖，成功即从队列移除。
    """
    if not _cycle_is_live(cfg):
        return
    positions = state.get("positions") or {}
    cache = book_cache()
    for pos_key, pos in list(positions.items()):
        pending = pos.get("pending_exit")
        if not isinstance(pending, dict) or not pending or pos.get("settled"):
            continue
        for token, rec in list(pending.items()):
            leg = next((lg for lg in (pos.get("legs") or [])
                        if str(lg.get("token_id") or "") == str(token) and not lg.get("settled")), None)
            if leg is None:
                del pending[token]
                continue
            channel = str((rec or {}).get("exit_channel") or "retry")
            if channel not in live_exit.EXIT_CHANNELS:
                # 退出重试必须落在既有白名单通道上，才能被审计行盖章；未知 ⇒ 归到最早的那条
                channel = live_exit.CH_SLEEVE_TIMEOUT if pos_key.endswith("#sleeve") else live_exit.CH_REFIRE_LIQ
            try:
                _live_exit_leg(cfg, state, pos, leg, cache, channel=channel, now_utc=now_utc)
            except Exception as exc:  # noqa: BLE001 — 单腿失败不得中断整轮（下一轮再试）
                log_event(cfg.get("log_path"),
                          {"type": "exit_retry_failed", "position_key": pos_key, "token_id": token,
                           "exit_channel": channel, "error": f"{type(exc).__name__}: {exc}",
                           "ts_utc": re_execution.iso_utc(now_utc)})
            if leg.get("settled"):
                (pos.get("pending_exit") or {}).pop(str(token), None)
        if isinstance(pos.get("pending_exit"), dict) and not pos["pending_exit"]:
...
def _expire_stale_sleeves(cfg: dict[str, Any], state: dict[str, Any], now_utc: datetime) -> None:
    ...
    for sess_key, rec in list(sleeves.items()):
        ...
        if leg is None:
            ...
            continue
        if _cycle_is_live(cfg):
            # live：**真实 FAK 卖**（撤真实挂单 → SELL，floor=live_sell_floor）。被拒/无买盘/
            # 深度不足/地板不满足 ⇒ 绝不标 settled、绝不标 liquidated：状态保持 open 下轮重试
            # （余量也留在这里续卖）。至结算仍未卖出则走既有 settle 兜底。
            close = _live_exit_leg(cfg, state, pos, leg, cache,
                                   channel=live_exit.CH_SLEEVE_TIMEOUT, now_utc=now_utc)
            if not close.get("sold"):
                rec["exit_deferred"] = str(close.get("reason") or "deferred")
                rec["exit_deferred_at_utc"] = re_execution.iso_utc(now_utc)
                continue
            if not close.get("leg_settled"):
                # 部分成交：余量留在仓位，下轮继续卖（rec 保持 open ⇒ 下轮仍会被扫到）
                rec["exit_deferred"] = "partial_fill_remaining"
                rec["exit_remaining_shares"] = close.get("remaining_shares")
                continue
        else:
...
def _paper_fire(
    ...
    for elapsed in (0, 1500, 4000):
        ...
        for intent in intents:
            ...
            # ③（2026-09-13）账本成本 = **实际成交**：``match["cost"]`` 现在是 transport 回报的
            # 实际成交（优先名义额 USDC，其次 股数 × 实际成交均价），**不再是** ``股数 × 限价``。
            # paper 通道由 ``paper_match_fak`` 按模拟 book 撮合，语义逐字未变。
            fills[intent["leg"]]["cost"] += match["cost"]
            if match.get("cost_source") == "unavailable" and (match["filled_shares"] or 0) > 0:
                # 有成交却读不到成交台账 ⇒ 成本不可知（本腿记 0，绝不回退限价）。**不静默**：
                # 落一条事件供对账/审计；宁可少记（可发现）也不按限价虚记（掩盖真实成本）。
                log_event(cfg.get("log_path"), {
                    "type": "fire_cost_unknown", "key": fire["key"], "leg": intent["leg"],
                    "filled_shares": str(match["filled_shares"]),
                    "detail": "venue trade ledger unreadable — leg cost booked 0 (never the limit)",
                })
    ...
    *,
    ...
    if old is not None and not old.get("settled") and filled_refire_yes:
        ...
        if old_yes is not None:
            ...
            exit_deferred: str | None = None
            if _cycle_is_live(cfg):
                # live：**真实 FAK 卖**旧桶 YES 腿（不是"按 bid 记账"）。被拒/无买盘/深度不足/
                # 地板不满足 ⇒ 绝不标 settled、绝不标 liquidated：仓位保持 open，余量登记
                # pending_exit，由下一轮的续卖扫（_retry_pending_exits）重试 —— 本通道只在
                # 追火那一刻触发一次，因此延期必须挂到队列上才有人兜。
                close = _live_exit_leg(cfg, state, old, old_yes, close_books,
                                       channel=live_exit.CH_REFIRE_LIQ, now_utc=now_utc)
                if not close.get("sold"):
                    exit_deferred = str(close.get("reason") or "deferred")
            else:
                close = close_leg_at_best_bid(
            ...
            if close is not None:
                try:
                    ...
                    cost = ZERO
                # live 延期（一股未成）不是"写零"：不记 loss，只记 exit_deferred（纯加法；
                # paper 下 exit_deferred 恒为 None ⇒ 这一行逐字不变）
                loss = None if exit_deferred else max(
                    ZERO, cost - Decimal(str(close["proceeds_usdc"])))
                log_event(cfg.get("log_path"), {
                    "type": "close_old_yes",
                    "key": sess,
                    "fire_no": int(fire.get("fire_no") or 2),
                    ...
                    "loss_usdc": str(loss) if loss is not None else None,
                    "closed_by": "refire_liquidation",
                    "book_unavailable": book_unavailable,
                    **({"exit_deferred": exit_deferred} if exit_deferred else {}),
                    "ts_utc": now_utc.isoformat(),
                })
    if old is not None:
...
def run_cycle(
    ...
    *,
    ...
    # 绑定本轮模式（live 真实 SELL 出场通道的前提；缺省 paper ⇒ 既有行为逐字不变）。
    # 守卫包装 ``close_leg_at_best_bid`` 靠它判断"这一轮是不是 live"，缺了它会退回 paper 语义。
    set_cycle_mode(cfg.get("mode"), log_path)
    if _cycle_is_live(cfg):
        # live：轮次序号 +1 —— ``live_sell_max_attempts_per_cycle`` 以"本轮同一腿的真实下单次数"
        # 为准（只在 live 下写状态 ⇒ paper 的 state/golden 一个键都不多）。
    ...
    for rule in rules_now:
        ...
        if cfg.get("strategy_mode") == "consensus_lock" and rule_books:
            ...
            if pos and (pos.get("settled") or pos.get("liquidated")):
                ...
                for l in pos.get("legs", []):
                    if l.get("outcome") == "YES" and Decimal(str(l.get("shares") or 0)) > ZERO:
                        strat.state.open_positions[rule_key] = PositionRecord(
                            ...
                            liquidated=False,
                            entry_channel=pos.get("entry_channel", ""),
                        )
                if pos.get("entry_channel"):
                    strat.state.locked_sessions[rule_key] = {"entry_channel": pos.get("entry_channel")}

                # ---- 50% 动态阶梯止盈 (Bid >= entry_avg_price * 1.50 卖出一半收回本金) ----
                tp_signal = strat.evaluate_take_profit(rule_key, pos, rule_books, now)
                if tp_signal and tp_signal.get("action") == "take_profit":
                    sell_shares = tp_signal["shares_to_sell"]
                    tp_res = {}
                    if _cycle_is_live(cfg):
                        for leg in pos.get("legs", []):
                            if leg.get("outcome") == "YES" and not leg.get("settled"):
                                res = _live_exit_leg(
                                    cfg, state, pos, leg, rule_books,
                                    channel=live_exit.CH_TAKE_PROFIT,
                                    shares_override=sell_shares,
                                    now_utc=now,
                                )
                                tp_res[leg.get("token_id")] = res
                                if res.get("sold"):
                                    pos["take_profit_done"] = True
                    else:
                        for leg in pos.get("legs", []):
                            if leg.get("outcome") == "YES" and not leg.get("settled"):
                                cur_sh = Decimal(str(leg.get("shares") or 0))
                                bid = tp_signal["best_bid"]
                                proceeds = (sell_shares * bid).quantize(Decimal("0.0001"))
                                rem_shares = cur_sh - sell_shares
                                leg["shares"] = str(rem_shares)
                                cost_base = Decimal(str(leg.get("cost_usdc") or 0))
                                if cur_sh > ZERO:
                                    sold_cost = (cost_base * (sell_shares / cur_sh)).quantize(Decimal("0.0001"))
                                    leg["cost_usdc"] = str(cost_base - sold_cost)
                                state["realized_pnl"] = str(Decimal(str(state.get("realized_pnl") or 0)) + proceeds - sold_cost)
                                if rem_shares <= Decimal("0.001"):
                                    leg["settled"] = True
                                pos["take_profit_done"] = True
                                tp_res[leg.get("token_id")] = {"status": "filled", "shares_sold": str(sell_shares), "proceeds": str(proceeds)}
                    log_event(log_path, {
                        "type": "take_profit_exit",
                        "key": rule_key,
                        "signal": {k: str(v) if isinstance(v, Decimal) else v for k, v in tp_signal.items()},
                        "result": tp_res,
                ...
                if early_stop and early_stop.get("action") == "early_stop_suppressed":
                    # ② 开仓冷静期（2026-09-13）：距入场不足 early_stop_grace_seconds ⇒ **不执行**
                    # 止损、不平仓、不熔断 —— 但**绝不静默**：落一条审计事件（含 would-be reason /
                    # bid / floor / 剩余冷静期 / session_key / entry_channel）。条件 A（surge）不经此路径。
                    # 去重（审计 M2 修复）：同一仓位在同一冷静期窗口内**只记一次**——strategy 以
                    # `duplicate` 标记"本轮已记过"。否则 1200s × ~20s/轮 会把审计刷成数十行噪声，
                    # 淹没 early_stop_suppressed 的信号价值（且使 duplicate 成为死代码）。
                    # 注意：去重只抑制**日志**，绝不改变抑制语义——不能让它掉进下面的 `elif early_stop:`
                    # 而误触发真实平仓/熔断。
                    if not early_stop.get("duplicate"):
                        log_event(log_path, {
                            "type": "early_stop_suppressed",
                            "key": rule_key,
                            "suppressed": dict(early_stop),
                            "ts_utc": re_execution.iso_utc(now),
                        })
                elif early_stop:
                    liq_res = {}
                    if _cycle_is_live(cfg):
                        # live：逐条 YES 腿**真实 FAK 卖**（撤真实挂单 → SELL，floor=live_sell_floor）。
                        # 只按**真实成交**更新账本：部分成交只减股数、余量留仓续卖；一股未成 ⇒
                        # exit_deferred、仓位保持 open；pos.liquidated 只在**全部 YES 腿了结**时置位。
                        for leg in pos.get("legs", []):
                            if leg.get("outcome") == "YES" and not leg.get("settled"):
                                res = _live_exit_leg(cfg, state, pos, leg, rule_books,
                                                     channel=live_exit.CH_EARLY_STOP, now_utc=now)
                                liq_res[leg.get("token_id")] = res
                        yes_live = [lg for lg in pos.get("legs", [])
                                    if lg.get("outcome") == "YES"]
                        if yes_live and all(lg.get("settled") for lg in yes_live):
                            pos["liquidated"] = True
                        ...
                        pos["liquidated"] = True
                        pos["liquidation_type"] = early_stop.get("reason")
                    # 止损后当日禁止再开仓（两种模式一致；沿用 stopped_out/breached 现状）
                    strat.state.stopped_out_sessions.add(rule_key)
                    strat.state.breached_sessions.add(rule_key)
                    log_event(log_path, {
        ...
        if cfg.get("strategy_mode") == "consensus_lock":
            ...
            # 记录 METAR 观测（无论是否持仓均维持温度序列供变率及夭折证伪复用）
            strat.record_metar_observation(rule_key, temp, obs, now)
            ...
            if pos and not pos.get("settled") and not pos.get("liquidated"):
                ...
                if breach:
                    ...
                    if _cycle_is_live(cfg):
                        # live：逐条 YES 腿**真实 FAK 卖**（撤真实挂单 → SELL，floor=live_sell_floor）。
                        # 被拒/无买盘/深度不足/地板不满足 ⇒ 绝不标 liquidated、绝不标 settled：
                        # 仓位保持 open，下一轮在破位仍成立时重试（余量同样续卖）。
                        for leg in pos.get("legs", []):
                            if leg.get("outcome") == "YES" and not leg.get("settled"):
                                _live_exit_leg(cfg, state, pos, leg, rule_books,
                    ...
                    # 气温不可逆夭折证伪判定 (日内高点暴跌 >= 2.5C 且 METAR 遇 CB/TS/RA)
                    abortion = strat.handle_weather_abortion_risk_control(
                        rule_key, city, rule.get("direction"), temp, obs, now
                    )
                    if abortion:
                        liq_res = {}
                        if _cycle_is_live(cfg):
                            for leg in pos.get("legs", []):
                                if leg.get("outcome") == "YES" and not leg.get("settled"):
                                    res = _live_exit_leg(
                                        cfg, state, pos, leg, rule_books,
                                        channel=live_exit.CH_ABORTION_RC,
                                        now_utc=now,
                                    )
                                    liq_res[leg.get("token_id")] = res
                            yes_live = [lg for lg in pos.get("legs", []) if lg.get("outcome") == "YES"]
                            if yes_live and all(lg.get("settled") for lg in yes_live):
                                pos["liquidated"] = True
                                pos["liquidation_type"] = "WEATHER_ABORTION"
                        else:
                            for leg in pos.get("legs", []):
                                if leg.get("outcome") == "YES" and not leg.get("settled"):
                                    close_leg_at_best_bid(
                                        state, leg, rule_books,
                                        closed_by="weather_abortion",
                                        settled_at_utc=re_execution.iso_utc(now),
                                    )
                            pos["settled"] = True
                            pos["liquidated"] = True
                        ...
                        strat.state.breached_sessions.add(rule_key)
                        log_event(log_path, {
                            "type": "weather_abortion_risk_control",
                            "key": rule_key,
                            "abortion_info": abortion,
                            "liquidation": liq_res,
                            "ts_utc": re_execution.iso_utc(now),
                        })
                continue
    ...
    #
    # live（2026-09-13）：续卖扫 —— 部分成交的余量 / 之前被拒而延期的退场，在下一轮真实卖一次。
    # 放在 sleeve 超时之前：先把上一轮的余量处理掉，再让本轮的超时逻辑读最新的 shares。
    _retry_pending_exits(cfg, state, now)
